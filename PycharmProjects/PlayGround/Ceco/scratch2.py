...
for ch_name in ch_list:
    ...
    # base_dir = os.path.join("D:\\Dump\\CecoComparision\\4.4-4.5\\UDP\\28-nov", ch_name)
    base_dir = os.path.join("D:\Dump\CecoComparision\\4.4-4.5\\PAL\\", ch_name, "15-Oct")
    ...
    # concat_ref_ceco_dir = os.path.join(base_dir, "Ref_Cecos_crop_tool", "crop", "concat")
    ...
    #             except Exception:
    #                 print("Error in deleting tmp vids{0}".format(tmp_vids_dir))
    #             detec_ch_util.create_ref_ceco(hour_mp4_file, hour_ref_ceco_file)
    # except Exception:
    #     print("Continue")
    ...
    for ceco_dir in ref_ceco_dir_lst:
        ...
        for ref_ceco in detec_ch_util.list_files_dir(ceco_dir, "ce"):
            ...
            hour_int = int(hour_dir)
            if hour_int > 5:
                date_obj = datetime.strptime("171128", '%y%m%d')
            else:
                date_obj = datetime.strptime("171129", '%y%m%d')
