...
for ch_name in ch_list:
    ...
    base_dir = os.path.join("D:\\Dump\\CecoComparision\\4.4-4.5\\NTSC\\", ch_name, "02-Nov")
    # base_dir = os.path.join("D:\\Dump\\CecoComparision\\4.4-4.5\\UDP\\02-Nov", ch_name)
    ...
    for ceco_dir in ref_ceco_dir_lst:
        ...
        for ref_ceco in detec_ch_util.list_files_dir(ceco_dir, "ce"):
            ...
            if int(hour_dir) > 13:
                date_obj = datetime.strptime("171101", '%y%m%d')
            else:
                date_obj = datetime.strptime("171102", '%y%m%d')
