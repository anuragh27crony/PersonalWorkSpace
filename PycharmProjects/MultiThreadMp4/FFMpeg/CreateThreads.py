...
def random_date():
    ...
    end_date="6/24/2016 1:00 AM"
    stime = time.mktime(time.strptime(start_date, format_string))
    etime = time.mktime(time.strptime(end_date, format_string))
...
def init_process():
    ...
    for iteration in range(86400):
        print("Round " + str(iteration))
        channel_range = [int(random.random() * 8 + 1)]
        print(channel_range)
        for channel in channel_range:
            current_thread_name=channel
            current_thread_data={"id":channel,"start_time":datetime.now(),"task":build_command(channel)}
            thread_queue.put(current_thread_data)
            current_thread = Thread(name=current_thread_name, target=ffmpeg_worker, args=(thread_queue, thread_result, thread_error))
